...
class PostgresConnection
{
    ...
    public function __construct(string $pgServiceName, string $pgServiceFile, Logger $logger)
    {
        try {
            ...
            $this->tableQueue = $config['table_queue'];
    ...
    /** Restituisce la tabella della coda */
    ...
    private function loadPgService(string $file, string $service): array
    {
        ...
        if (!isset($config['table_queue'])) {
            throw new \RuntimeException("Chiave 'table_queue' non definita in {$service}");
        }
